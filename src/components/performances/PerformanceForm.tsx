...
const formSchema = z.object({
  name: z.string({ required_error: "필수 항목" }).min(2, { message: "최소 2자" }).max(50, { message: "최대 50자" }),
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 w-full">
              ...
              <SimpleStringField
                ...
                required={!(formSchema.shape.description instanceof z.ZodOptional)}
              />
              <SimpleImageField
                form={form}
                name="representativeImage"
                label="대표 이미지"
                required={!(formSchema.shape.representativeImage instanceof z.ZodOptional)}
              ...
              <SimpleDateField
                ...
                name="start_datetime"
                label="시작 일시"
                required={!(formSchema.shape.start_datetime instanceof z.ZodOptional)}
              />
              <SimpleDateField
                form={form}
                name="end_datetime"
                label="종료 일시"
                required={!(formSchema.shape.end_datetime instanceof z.ZodOptional)}
              />

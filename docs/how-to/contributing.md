...
이슈 → 브랜치 → PR 워크플로와 이슈/PR/커밋 컨벤션을 정의합니다.
...
이슈 제목은 **자연어**로 작성합니다. Conventional Commits(`fix(...)`, `feat(...)`) 포맷을 쓰지 않습니다 — CC는 _변경 행위_(커밋/PR)를 분류하는 도구이고, 이슈는 *문제/요청*을 기술하는 대상이라 본질이 다릅니다.

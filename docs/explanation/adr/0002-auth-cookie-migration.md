...
- **preview의 서버측 인증 게이트 우회**: cookie의 `Domain`이 staging 스코프라 preview 오리진으로 오는 요청에는 브라우저가 cookie를 붙이지 않는다 — preview의 proxy(Edge)·RSC는 로그인 상태를 볼 방법 자체가 없다. 게이트를 그대로 두면 로그인해도 보호 라우트(/admin, /profile 등)가 /login으로 튕겨 preview 검토가 막히므로, 요청 호스트가 cookie `Domain` 스코프 밖(`*.vercel.app`)일 때 서버측 게이트를 건너뛰고 판정을 클라이언트(`useAuth`)에 위임한다 ([apps/web/lib/auth/authCookieScope.ts](../../../apps/web/lib/auth/authCookieScope.ts)). 판정 기준을 `VERCEL_ENV`가 아니라 **호스트**로 두는 이유: staging은 preview 배포에 커스텀 도메인을 alias한 형태([deploy-staging.yml](../../../.github/workflows/deploy-staging.yml))라 `VERCEL_ENV === "preview"`인데도 cookie 스코프 안이고, 게이트가 정상 동작해야 하는 환경이다. 권한 방어는 API의 `JwtAuthGuard`·`AdminGuard`가 담당하므로 우회로 열리는 것은 UI 껍데기뿐 — 데이터 요청은 401/403. 대신 preview에서는 리다이렉트 동작 자체를 검증할 수 없다.

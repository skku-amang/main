...
- 진행 중 아이디어의 세부 태스크는 GitHub 이슈로 관리한다. 아이디어 row에 안정적 **ID**(Notion unique_id, 예 `IDEA-12`)를 두고, GitHub 이슈가 그 ID를 참조한다(1 아이디어 → N 이슈). 별도 마일스톤은 만들지 않는다 — 진행률은 그 ID로 이슈를 검색(`IDEA-12 in:title is:open` → 0건이면 완료).
...
- **좋은 결과**: 팀 전원이 Notion에서 아이디어 제안·우선순위 참여. 자동 랭킹으로 "무엇부터" 판단 용이. 구현은 GitHub 네이티브(이슈·PR)로 유지. 아이디어당 GitHub 아티팩트(마일스톤 등) 생성 부담 없음 — ID 참조만.
- **나쁜 결과**: Notion 상태와 GitHub 진행이 걸쳐, 진행 중→완료 전이를 수동으로 맞춰야 함(해당 ID의 열린 이슈가 0인지 검색해 Notion을 옮김). 마일스톤 자동 완료율 바는 없다(검색으로 대체). 완전 자동 동기화는 하지 않는다.

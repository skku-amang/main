...
function getRandomValueByProbability(probability: { [key: string]: number }): string {
  // 확률밀도함수 정의
...
function createMemberSessions(): MemberSession[] {
  ...
  Object.entries(probability).map(([session, p]) => {
    ...
    const members = Array.from({ length: Number(memberCount) }, () => createUser(Math.floor(Math.random() * 100) + 1));

    result.push({
      session: dummySessions.find((s) => s.name === sessionName)!,
      members,
      requiredMemberCount: members.length + (Math.random() < 0.2 ? 1 : Math.random() < 0.1 ? 2 : 0)
    });
  })

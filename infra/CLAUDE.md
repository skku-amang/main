...
루트 [CLAUDE.md](../CLAUDE.md)와 함께 읽는다. 본 문서는 **AMANG 프로젝트 전용 인프라 컨벤션**만 담는다 (K8s 일반 베스트 프랙티스나 Terraform 일반 룰은 별도 참조).
...
- 잘못된 cluster cert로 sealing 방지: kubeseal 실행 전 `kubectl config current-context`로 cluster 확인. cluster 식별이 헷갈리면 ArgoCD UI의 amang Application `destination.server`로 확인 (homelab cluster)
...
PR 머지 = 배포 완료 아님. ArgoCD가 git → cluster sync까지 마쳐야 검증 시작. **hard-refresh로 reconcile 강제** 후 동기 wait:

```bash
# hard refresh 먼저 (Argo poll interval ~3분이라 stale state로 false-positive 위험)
...
manifest-only 변경은 동기 검증. 이미지 빌드 거치는 변경(Dockerfile/앱 코드)은 GHA 빌드(~1분) + Image Updater poll(평균 1분) + write-back + rollout까지 ~3-5분 비동기 갭 발생.

## Terraform

- 현재 [terraform/vercel/](terraform/vercel/)만 운영. AWS는 미정
- `terraform.tfvars` 커밋 금지 (`.gitignore` 등록), `terraform.tfvars.example`만 커밋
- `terraform apply -auto-approve` 금지 — 항상 `plan` 확인 후 수동 승인
